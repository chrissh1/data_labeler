...
- Structure: one narrow working column. Small heading, instructions/results links, progress, message, native radio choices, Back and Save & next. Results use a wider table and stacked rows on small screens.
...
- Progress: a compact count and bar above the current message; every successful save advances it.
- Motion: none. Loading and errors use inline text; successful saves advance the message.
- Tutorial: a short native dialog, shown once per browser, always reopenable.
- Results: a normal link opens a separate browser tab. Refresh explicitly to see newer labels.
